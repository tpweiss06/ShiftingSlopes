...
load("SimData/DispInitNew.rdata")
...
pdf(file = PlotName, width = FigWidth, height = FigHeight, onefile = FALSE, paper = "special")
     ...
     for(i in SimSeq){
          AllVals <- log(c(DispInit[[i]][[CurSpeed]]$Extant, DispInit[[i]][[CurSpeed]]$Extinct), base = 10)
          AllHist <- hist(AllVals, plot = FALSE, breaks = HistBreaks)
          plot(AllHist, col = AllCol, main = "", xlab = "", ylab = "", cex.axis = AxisSize,
               las = 1, xlim = xRange, xaxt = "n")
          if(NumExtant[i] > 0){
               hist(log(DispInit[[i]][[CurSpeed]]$Extant, base = 10), col = ExtantCol, breaks = HistBreaks, add = TRUE)
          }
