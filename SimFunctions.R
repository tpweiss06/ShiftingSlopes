...
Disperse <- function(traits, width, kern, PatchScale = 1, angles, AngleIndex,
                     CurPop, PopSize, PopIndices, NumRands, PopMat){
     # First generate the dispersal distances according to the type of kernel
     if(kern == "norm"){
          sigma <- (traits[CurPop,3]^2 * pi) / 2
          dists <- abs(rnorm(n = PopSize, mean = 0, sd = sigma))
     } else if(kern == "exp"){
          dists <- rexp(n = PopSize, rate = 1 / traits[CurPop,3])
     } else if(kern == "stud_t"){
          dists <- rStudKern(n = PopSize, d = traits[CurPop,3])
     }
...
RelFit <- function(LocalSel, tau, beta, traits, PopMat, individuals, omega,
     ...
     #    are in space.
     numerator <- exp(lambda * (PopMat[individuals, PopIndices$x1] - beta))
     denominator <- 1 + exp(lambda * (PopMat[individuals, PopIndices$x1] - beta))
...
# nDisp:            Number of dispersal loci
# nFit:             Number of fitness loci
...
MatFill <- function(RealizedNtp1, PopIndices, OccPatches, SexRatio, RelFits,
...
# PatchScale:       A factor determining the mapping of individual patches onto
...
# nDisp & nFit:     Number of loci for each trait
...
Reproduce <- function(beta, gamma, tau, omega, R0, K0, 
     ...
     for(i in 1:NumPatches){
          ...
          if( (PatchR == 0) & (PatchK == 0)){
               ExpPopGrowth <- 0
          } else if(PatchPopSize < (PatchK * (1 + 1/PatchR))){
               ExpPopGrowth <- 1 + PatchR * (1 - PatchPopSize / PatchK)
          } else{
               ExpPopGrowth <- 0
          }
          ...
          MeanFit <- mean(RelFits[CurPatchPop])
          AdjPopGrowth <- MeanFit * ExpPopGrowth * PatchPopSize
          ...
          if( !(is.null(PopIndices$sex)) ){
               NumFemales <- length(which( (PopMat[CurPatchPop,PopIndices$sex] == 1) ))
               NumMales <- length(which( (PopMat[CurPatchPop,PopIndices$sex] == 0) ))
...
Initialize <- function(PopIndices, PopSize, BetaInit, width, 
                       SexRatio = 0.5, FitInit, FitDiv, DispInit, DispDiv, NumCols){
     ...
     if( !(is.null(PopIndices$sex)) ){
          PopMat[,PopIndices$sex] <- rbinom(n = PopSize, size = 1, prob = SexRatio)
     }
...
PopMatColNames <- function(nFit, nDisp, monoecious, example = FALSE){
     ...
     PopIndices$x1 <- 3
     PopIndices$y1 <- 4
     PopIndices$FitCols <- 5:(5+2*nFit-1)
     PopIndices$DispCols <- (5 + 2*nFit):(5 + 2*nFit + 2*nDisp - 1)
     if(monoecious){
          PopIndices$sex <- NULL
     } else{
          PopIndices$sex <- 5 + 2*nFit + 2*nDisp
     }
...
SaveParams <- function(parameters, FilePath){
     ...
     cat("SexRatio <- ", parameters$SexRatio, "\n", sep = "")
     sink()
}
...
FullSim <- function(parameters, parallel = FALSE, SumMatSize = 5000, PopInit = NULL){
     ...
     FitPerLocusProb <- U[1] / (2*nFit)
     FitMutStd <- sqrt(Vm[1] / U[1])
     DispPerLocusProb <- U[2] / (2*nDisp)
     DispMutStd <- ifelse(U[2] == 0, 0, sqrt(Vm[2] / U[2]))
     FitNumMut <- rbinom(n = NumRands, size = nFit, prob = FitPerLocusProb)
     DispNumMut <- rbinom(n = NumRands, size = nDisp, prob = DispPerLocusProb)
     if( !(is.null(PopIndices$sex)) ){
          SexRands <- rbinom(n = NumRands, size = 1, prob = SexRatio)
     }
     OffspringIndex <- 1
     ...
     RelFits <- RelFit(LocalSel = LocalSel, tau = tau, beta = BetaInit, traits = traits, 
     ...
     RealizedNtp1 <- Reproduce(beta = BetaInit, gamma = gamma, tau = tau, omega = omega,
                         R0 = R0, K0 = K0, traits = traits, PopMat = PopMat, EnvGradType = EnvGradType,
                         PopIndices = PopIndices, SexRatio = 0.5, PatchScale = PatchScale,
                         SexRands = SexRands, CurPop = CurPop, nDisp = nDisp, nFit = nFit, 
     ...
     if(SumNtp1 > 0){
          PopMat <- MatFill(RealizedNtp1 = RealizedNtp1, PopIndices = PopIndices,
                             ...
                             CurPop = CurPop, nDisp = nDisp, nFit = nFit, DispSegVals1 = DispSegVals1,
     ...
     for(g in 2:TotalTime){
          ...
          if(PopSize != 0){
               ...
               Deltas <- Disperse(traits = traits, width = width, kern = kern, PatchScale = PatchScale,
               ...
               if(SumNtp1 > 0){
                    ...
                    if( (FitSegIndex + SumNtp1*nFit) > NumRands ){
                         FitSegVals1 <- sample(c(0,1), replace = TRUE, size = NumRands)
                         FitSegVals2 <- sample(c(0,1), replace = TRUE, size = NumRands)
                         FitSegIndex <- 1
                    }
                    if( (DispSegIndex + SumNtp1*nDisp) > NumRands ){
                         DispSegVals1 <- sample(c(0,1), replace = TRUE, size = NumRands)
                         DispSegVals2 <- sample(c(0,1), replace = TRUE, size = NumRands)
                         DispSegIndex <- 1
                    }
