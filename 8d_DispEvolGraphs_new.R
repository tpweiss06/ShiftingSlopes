...
load("SimData/DispEvolveNew.rdata")
...
NumExtant <- matrix(NA, nrow = 9, ncol = 3)
for(i in 1:9){
     CurSims <- subset(SimIDs, Params == i)
     for(j in 1:3){
          NumExtant[i,j] <- sum(CurSims[,j+2])
     }
}
...
for(v in 1:3){
     ...
     pdf(file = PlotName, width = FigWidth, height = FigHeight, onefile = FALSE, paper = "special")
          ...
          for(i in SimSeq){
               ParamData <- c(DispEvolve[[i]][[v]]$Extant, DispEvolve[[i]][[v]]$Extinct)
               ExtantData <- DispEvolve[[i]][[v]]$Extant
               ParamHist <- hist(ParamData, plot = FALSE, breaks = HistBreaks)
               plot(ParamHist, col = AllCol, main = "", xlab = "", ylab = "", 
                    las = 1, xlim = xRange, xaxt = "n", yaxt = "n")
               axis(1, cex = AxisSize)
               axis(2, cex.axis = AxisSize)
               if(NumExtant[i,v] > 0){
                    hist(ExtantData, col = ExtantCol, breaks = HistBreaks,
                         add = TRUE)
               }
               # Add in the number of extant simulations
               SimMessage <- paste("n = ", NumExtant[i,v], sep = "")
