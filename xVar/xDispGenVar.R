# This script will extract the genetic variance in dispersal throughout the range
     ...
     for(g in GenSeq){
          ...
          for(s in 1:nrow(CurSims)){
               ...
               for(j in xSeq){
                    ...
                    Temp <- rbind(c(j, sd(LocalData$DispGenVar)), Temp)
               }
          }
...
DispGenVarData <- vector(mode = "list", length = 9)
for(p in 1:9){
     DispGenVarData[[p]] <- vector(mode = "list", length = 3)
     for(v in 1:3){
          CurIndex <- which((SpeedVec == SpeedWords[v]) & (ParamVec == p))
          CurResults <- SimGenVar[[CurIndex]]
          DispGenVarData[[p]][[v]] <- CurResults
     }
}

# Save the output
save(DispGenVarData, file = "~/ShiftingSlopes/xVarDispGenVarData.rdata")
