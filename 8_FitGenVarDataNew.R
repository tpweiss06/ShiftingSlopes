...
GenVarExtract <- function(i){
     ...
     for(g in GenSeq){
          ...
          for(s in 1:nrow(CurSims)){
               ...
               for(j in xSeq){
                    ...
                    if(SpeedWord == "Slow"){
                         if(CurSims$Slow[s] == 1){
                              TempExtant <- rbind(c(j, mean(LocalData$FitGenVar)), TempExtant)
                         }else{
                              TempExtinct <- rbind(c(j, mean(LocalData$FitGenVar)), TempExtinct)
                         }
     ...
     ExtantGenVar <- ExtantDisps[-nrow(ExtantGenVar),]
...
# Save the output
save(FitGenVarData, file = "~/ShiftingSlopes/FitGenVarDataNew.rdata")
